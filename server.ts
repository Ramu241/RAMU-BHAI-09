...
async function startServer() {
  ...
  // Proxy API route to avoid CORS when fetching Bingo 30S history
  app.all("/api/bingo-history-30s", async (req, res) => {
    try {
      ...
      try {
        ...
        console.error("Failed to parse response text as JSON for 30s:", text.substring(0, 100));
        return res.status(502).json({ error: "Invalid JSON from target" });
      }

      return res.json(data);
    } catch (error: any) {
      console.error("Error proxying 30s bingo history:", error);
